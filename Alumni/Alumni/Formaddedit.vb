...
Public Class Formaddedit
    ...
    Private Sub lvRecords_ItemSelectionChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.ListViewItemSelectionChangedEventArgs) Handles lvRecords.ItemSelectionChanged
        ...
        selAge = lvRecords.FocusedItem.SubItems(2).Text
        selBirthDate = lvRecords.FocusedItem.SubItems(3).Text
        selCivilStatus = lvRecords.FocusedItem.SubItems(4).Text
        selHomeAddress = lvRecords.FocusedItem.SubItems(5).Text
        selCityMunicipality = lvRecords.FocusedItem.SubItems(6).Text
        selContactNumber = lvRecords.FocusedItem.SubItems(7).Text
        selEmail = lvRecords.FocusedItem.SubItems(8).Text
        selTribe = lvRecords.FocusedItem.SubItems(9).Text
        selReligion = lvRecords.FocusedItem.SubItems(10).Text
        selEA = lvRecords.FocusedItem.SubItems(11).Text
        selTradeArea = lvRecords.FocusedItem.SubItems(12).Text
        selPhase = lvRecords.FocusedItem.SubItems(13).Text
        selYearGraduated = lvRecords.FocusedItem.SubItems(14).Text
    ...
    Private Sub btnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEdit.Click
        If Not selFirstName = "" Then
            grp_Student.Text = "Update Employee"
        ...
        Else
            MsgBox("Please Select a Employee", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly)
        End If
    ...
    Private Function addStudent(ByVal vFname As String, ByVal vSex As String, ByVal vStat As String, ByVal vAge As String, ByVal vBday As String, ByVal vHomeA As String, ByVal vCity As String, ByVal vTribe As String, ByVal vTA As String, ByVal vReligion As String, ByVal vNum As String, ByVal vYearg As String, ByVal vEA As String, ByVal vEmail As String, ByVal vPhase As String) As Boolean
        ...
        If myConn.State = ConnectionState.Open Then
            ...
            myCommand.CommandText = "INSERT INTO vsbt(Fname, Sex, Stat, Age, Bday, HomeA, City, Tribe, TA, Religion, Num, Yearg, EA, Email, Phase) VALUES('" & vFname & "','" & vSex & "','" & vStat & "','" & vAge & "','" & vBday & "','" & vHomeA & "','" & vCity & "','" & vTribe & "','" & vTA & "','" & vReligion & "','" & vNum & "','" & vYearg & "','" & vEA & "','" & vEmail & "','" & vPhase & "')"
...
ErrTrap:
    ...
    Private Function updateStudent(ByVal vFname As String, ByVal vSex As String, ByVal vStatus As String, ByVal vAge As String, ByVal vBday As String, ByVal vHomeA As String, ByVal vCity As String, ByVal vTribe As String, ByVal vTA As String, ByVal vReligion As String, ByVal vNumber As String, ByVal vYearg As String, ByVal vEA As String, ByVal vEmail As String, ByVal vPhase As String) As Boolean
        ...
        If myConn.State = ConnectionState.Open Then
            myCommand.Connection = myConn
            myCommand.CommandText = "UPDATE vsbt SET Fname = '" & vFname & "', Sex = '" & vSex & "', Stat = '" & vStatus & "', Age = '" & vAge & "', Bday = '" & vBday & "', HomeA = '" & vHomeA & "', City = '" & vCity & "', Tribe = '" & vTribe & "', TA = '" & vTA & "', Religion = '" & vReligion & "', Num = '" & vNumber & "', Yearg = '" & vYearg & "', EA = '" & vEA & "', Email = '" & vEmail & "', Phase = '" & vPhase & "'WHERE Fname = '" & selFirstName & "'; "
            myAdapter.SelectCommand = myCommand
            myCommand.ExecuteReader()
        End If
        myConn.Close()
...
Errtrap:
    ...
    Private Sub btnOk_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOk.Click
        If btnOk.Text = "Add" Then
            ...
            If addStudent(TxtFname.Text, CBSex.Text, CBStatus.Text, TxtAge.Text, DTPBirth.Text, TxtHomeA.Text, TxtCity.Text, TxtTribe.Text, TxtTradeArea.Text, TxtReligion.Text, TxtNumber.Text, TxtYearg.Text, TxtEA.Text, TxtEmail.Text, TxtPhase.Text) = True Then
                MsgBox("New Customer Successfully Added!", MsgBoxStyle.Information + MsgBoxStyle.OkOnly)
                Me.loadRecords()
